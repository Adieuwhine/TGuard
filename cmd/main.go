...
import (
	...
	"tguard/internal/logger"
    "tguard/internal/service"
)
...
func main() {
	...
	uninstallCmd := &cobra.Command{
		Use:   "uninstall",
		Short: "Удалить все изменения, внесённые tguard",
		Long:  `Удаляет цепочки iptables/ipset, systemd сервисы и конфигурационные файлы, созданные tguard.`,
		Run:   runUninstall,
	}
	uninstallCmd.Flags().BoolVar(&confirmYes, "yes", false, "Подтвердить удаление без интерактивного запроса")
	uninstallCmd.Flags().BoolVar(&removeLogs, "remove-logs", false, "Удалить логи tguard из /var/log")
...
func runFull(cmd *cobra.Command, args []string) {
	...
	if cmdSvc.CommandExists("ufw") {
		...
		if !isActive {
			...
			log.Warn().Msg("")
			log.Warn().Msg("tguard проверит наличие правил SSH и прервёт установку если их нет")
			log.Warn().Msg("")
		}
	}
...
func runUninstall(cmd *cobra.Command, args []string) {
	...
	if !confirmYes {
		fmt.Print("Это удалит правила tguard, systemd-сервисы и конфигурацию. Продолжить? [y/N]: ")
		if !confirmFromStdin() {
