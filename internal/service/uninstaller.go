...
func (s *UninstallerService) Uninstall(removeLogs bool) error {
	s.logger.Info().Msg("=== Uninstall TGuard ===")
	s.logger.Info().Msg("TGuard does not modify Linux routing tables (ip rule/ip route), skipping routing rollback")
	...
	s.logger.Info().Msg("TGuard uninstall completed")
	return nil
}
...
func (s *UninstallerService) removeManagedUFWBlock(path string) (bool, error) {
	...
	s.logger.Info().Str("path", path).Int("blocks", blocksRemoved).Msg("Removed TGuard managed UFW blocks")
	return true, nil
}
